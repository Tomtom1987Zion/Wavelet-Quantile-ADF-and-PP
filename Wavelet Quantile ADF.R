##=======Wavelet Quantile Augmented Dickey-Fuller (Wavelet QADF) ========================##
...
# Define a function to perform quantile ADF test
quantile_adf_test <- function(y, tau) {
  ...
  for (i in 1:length(tau)) {
    ...
    result <- ur.df(psiy, type = "drift", lags = 1)
...
# Define a function to perform Wavelet Quantile ADF test with different levels
wavelet_quantile_adf_tests <- function(variable_name) {
  ...
  # Perform quantile ADF tests for each level
  adf_short <- quantile_adf_test(short_run, tau)
  adf_medium <- quantile_adf_test(medium_run, tau)
  adf_long <- quantile_adf_test(long_run, tau)
  
  # Combine results into data frames with Run labels
  df_adf <- data.frame(
    Quantile = tau,
    Test_Statistic = c(adf_short, adf_medium, adf_long),
...
# Apply the Wavelet Quantile ADF tests to each variable (LEG, LFD, LIQ, LREC, LTI)
results_LEG <- wavelet_quantile_adf_tests("LEG")
results_LFD <- wavelet_quantile_adf_tests("LFD")
results_LIQ <- wavelet_quantile_adf_tests("LIQ")
results_LREC <- wavelet_quantile_adf_tests("LREC")
results_LTI <- wavelet_quantile_adf_tests("LTI")
...
p <- ggplot(results_all, aes(x = Quantile, y = Test_Statistic, color = Run, shape = Run, group = interaction(Variable, Run))) +
  ...
  geom_hline(yintercept = -3.43, linetype = "dotted", color = "red", size = 1) +  # 1% critical value
  geom_hline(yintercept = -2.86, linetype = "dashed", color = "blue", size = 1) +  # 5% critical value
  geom_hline(yintercept = -2.57, linetype = "solid", color = "green", size = 1) +  # 10% critical value
  labs(title = "Wavelet Quantile ADF Test Results",
